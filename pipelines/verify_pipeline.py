...
_ZK_ROOT = Path(__file__).resolve().parents[1]
_GRAPH_ROOT = _ZK_ROOT.parent
...
from core.models import ModelRegistry, build_registry
...
def run_verify(
    ...
    for model_name in model_names:
        ...
        _progress(f"[{model_name}] 开始推理（{num_steps} 步，每步 {step_h}h）")

        # 初始化模型状态
        try:
            state = model.init_state(init_blob, prev_blob=prev_blob, init_dt=init_dt)
        except ValueError as e:
            _progress(f"[{model_name}] init_state 失败: {e}")
            continue

        # 决定输出变量
        sfc_vars = variables if variables else model.get_surface_var_names()

        out_dir = Path(output_root) / model_name / f"{date}T{hour:02d}"
        ...
        for step in range(1, num_steps + 1):
            ...
            _progress(f"[{model_name}] step={step}, lead={lead}h, valid={valid_dt.strftime('%Y-%m-%dT%H')}")
                    ...
                    for pvar, levels in pressure_vars.items():
                        ...
                        for lev in levels:
                            ...
                            plot_compare(
                                ...
                                pred_field,
                                truth_field,
                                title=f"{model_name} +{lead}h {pvar}@{lev}hPa | {date}T{hour:02d}",
                                cmap="viridis",
                            )

        _progress(f"[{model_name}] 推理验证完成，图像保存至: {out_dir}")
